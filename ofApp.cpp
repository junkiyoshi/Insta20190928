...
void ofApp::draw() {
	...
	for (int i = 0; i < 4; i += 1) {
		...
		for (int x = -120; x < 120; x += this->font_size) {
			...
			for (int y = -120; y < 120; y += this->font_size) {
				...
				int charactor_index = ofMap(ofNoise(noise_location.x * 0.001, noise_location.y * 0.001, noise_location.z * 0.001, ofGetFrameNum() * 0.005), 0, 1, 0, charactors.size());
